...
=====================================================
In Fig. 4 of the main text where we show the uncertainty around our models' forecasts to a 1% change in climate covariates, the obvious outlier is *Pascopyrum smithii* (PASM). The uncertainty around PASM forecasts from the integral projection model is orders of magnitude larger than for any other species, and is even larger than the uncertainty around the quad-based model forecasts. It turns out that the recruitment parameters we estimate lead to overcompensating density-dependence that is amplified when those parameters are allowed to vary. When we allow all parameters to vary except those associated with recruitment, uncertainty around PASM decreases to levels similar to those associated with other species (Fig. C1). So detailed demographic data is not a cure-all, and can sometimes lead to interesting dynamics when parameter uncertainty is considered.

```{r plot_it, echo=FALSE, message=FALSE, warning=FALSE, include=TRUE, fig.width=5, fig.cap="Equilibrium cover and 90% quantiles around the mean prediction from the IPM when model error and parameter uncertainty are propogated through the simulation phase for all vital rates except recruitment."}
library(plyr)
library(ggplot2)
