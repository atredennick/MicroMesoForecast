...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \chead{\color{red}\textsf{MANUSCRIPT IN PRESS AT \emph{METHODS IN ECOLOGY AND EVOLUTION}}}
- \lhead{}
- \rhead{}
layout: 11pt
linkcolor: black
...
\text{logit}(s_{i,q,t}) &= \beta_{0,t} + \beta_{s,t}x_{i,q,t} + \beta_{Q,q} + \textbf{z}'_t \boldsymbol{\beta}_c \\
&+ \beta_{d,1} w_{i,t} + \beta_{d,2} (x_{i,q,t}w_{i,q,t}),
\end{align}
...
\mu_{i,q,t+1} &= \beta_{0,t} + \beta_{s,t}x_{i,q,t} + \beta_{Q,q} + \textbf{z}'_t \boldsymbol{\beta}_c \\
&+ \beta_{d,1} w_{i,q,t} + \beta_{d,2} (x_{i,q,t}w_{i,q,t}),
\end{align}
...
\begin{align}
k_{j}(v_{j},u_{j},\bar{w_{j}}) &= S_j(u_j, \bar{w_{j}}(u_{j}))G_j(v_{j},u_{j},\bar{w_{j}}(u_{j})) \\
&+ R_j(v_{j},u_{j},\bar{w_{j}}),
\end{align}
...
However, only for *B. gracilis* were the skill increases statistically significant at $\alpha$ = 0.05 for the IPM ($t_{(279)}$ = 1.70, *P* = 0.045) and the QBM ($t_{(279)}$ = 1.80, *P* = 0.037).
