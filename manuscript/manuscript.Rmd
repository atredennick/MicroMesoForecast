...
p1 <- ggplot(subset(resDall, species=="BOGR"))+
  geom_hline(aes(yintercept=0), linetype=2)+
  geom_boxplot(aes(x=year, y=resids, fill=simtype), outlier.shape = 1, outlier.size = 0, alpha=0.75, color="grey25")+
  xlab("Year")+
  ylab("Residuals (% cover difference)")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values=c("darkgoldenrod1", "steelblue4"))+
...
p2 <- ggplot(subset(resDall, species=="HECO"))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values=c("darkgoldenrod1", "steelblue4"), labels=c("Year effects", "No year effects"))+
