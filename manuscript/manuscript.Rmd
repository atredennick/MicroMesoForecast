...
# clim_id <-substr(cover_files, 16, )
# clim_id <- rep(c("Ppt", "PptTemp", "Temp",
#                  "Ppt", "ZPptTemp", "Temp", 
#                  "Ppt", "ZPptTemp","Temp"), 4)
...
for(i in 1:num_files){
  ...
  tmpid <- substr(cover_files[i], 16, nchar(cover_files[i])-4)
  tmpid <- strsplit(tmpid, "(?!^)(?=[[:upper:]])", perl=T)
  if(length(tmpid[[1]])==3){
    ttt <- paste(tmpid[[1]][[1]], tmpid[[1]][[2]], sep="")
    tmpid[[1]] <- c(ttt, tmpid[[1]][3])
  }
  tmp$species <- rep(spp_id[i], nrow(tmp))
  tmp$sim <- rep(tmpid[[1]][2], nrow(tmp))
  tmp$vital <- rep(tmpid[[1]][1], nrow(tmp))
