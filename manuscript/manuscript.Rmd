...
resD <- readRDS("../montana/quadBM/simulations/oneStepResultsFull.rds")
nSims <- length(unique(resD$sim))
diffD <- ddply(resD, c("year", "species"), summarise,
               avgDiff = mean(resids),
               sdDiff = sd(resids),
               dnDiff = quantile(resids, probs = 0.025, names=FALSE),
               upDiff = quantile(resids, probs = 0.975, names=FALSE))
ggplot(diffD)+
  geom_hline(aes(yintercept=0), linetype=2, color="grey50")+
  geom_point(aes(x=year, y=avgDiff), size=3)+
#   geom_errorbar(aes(x=year, ymax=avgDiff+sdDiff, ymin=avgDiff-sdDiff), width=0.1)+
    geom_errorbar(aes(x=year, ymax=upDiff, ymin=dnDiff), width=0.1)+
...
#   stat_smooth(data=diffD, aes(x=year, y=avgDiff, group=species), color="lightblue", se = FALSE, size=1)+
  geom_boxplot(aes(x=year, y=resids), outlier.shape = 1, width=0.3, outlier.size = 0.5)+
  facet_grid(species~., scales = "free_y")+
  xlab("Year")+
  ylab("Residuals (% cover difference)")+
  theme_bw()+
