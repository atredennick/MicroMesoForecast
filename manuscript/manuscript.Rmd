...
stats_table <- stats_table[,c("model", "species", "mae",
                              "rho","mean_cover")]

colnames(stats_table) <- c("Model", "Species", "MAE", "Accuracy", "Mean Obs. Cover")
print.xtable(xtable(stats_table, caption = "Mean absolute error (MAE) and accuracy (Pearson's $\\rho$) for one-step-ahead forecast from both model types. Forecasts were made without random year effects; only climate covariates could explain year-to-year variation"),type="latex", comment=FALSE, include.rownames=FALSE)
```
