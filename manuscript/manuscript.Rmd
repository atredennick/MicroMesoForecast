...
However, for one species, forecast skill increased when we linked demographic data to climate covariates.
We conclude that models based on aggregated individual-level data offer a practical alternative to data-intensive demographic models when species do not respond strongly to interannual variation in weather, but when species do respond to climate drivers, demographically-based models can generate more skillful forecasts.
...
IPM forecasts were significantly more accurate than the QBM in only one case (Fig. 2): forecast accuracy of *Poa secunda* percent cover from an IPM with climate covariates was greater than the accuracy from the QBM with climate covariates ($t_{(195)}$ = 1.72, *P* = 0.043).
...
The only exception is for *Poa secunda*, where IPM forecast accuracy remained steady as the forecast horizon increases, after an initial decrease from a forecast horizon of one year, whereas QBM forecast accuracy declined (Fig. 4).
