...
Figures
-------
```{r figure_1, dependson="plot-options", fig.cap="Boxplots of model residuals for one-step-ahead forecasts at each observation year. Each one-step forecast was simulated `r nSims` times. Note that the y-axes vary across panels.", fig.width=8, fig.height=8, cache=FALSE, results='hide'}
...
p4 <- ggplot(subset(resDall, species=="POSE"))+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_fill_manual(values=c("darkgoldenrod1", "steelblue4"))+
  ggtitle(expression(italic("P. secunda")))+
  guides(fill=FALSE)+
