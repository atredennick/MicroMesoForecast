...
However, it is unknown whether models built using population-level data adequately capture the effects of density-dependence and environmental forcing that are necessary to generate skillful forecasts.
        ...
        \item For our focal species at this particular location, and using our particular statistical models, percent cover models generated forecasts as skillful as those from a demographic model. We conclude that models based on aggregated individual-level data offer a practical alternative to data-intensive demographic models when forecasting is the goal. Percent cover data are widely collected for many plant species, and several long-term time series already exist. Our results should inspire plant population modelers to exploit such data to forecast the impacts of climate change.
    \end{enumerate}
...
This is especially important when forecasting the impacts of climate variability, where it is important to include to effects of forcing variables (e.g., temperature and precipitation) even of such effects are difficult to identify.
...
Our comparison of two model types, one based on individual-level data and one based on percent cover data, should not be considered an exhaustive test.
There are several reasons why the percent cover-based model generated equally skillfull forecasts for our focal species, and several reasons why percent cover models may fail in other cases.
First, we had no cases where a climate covariate had a strong negative effect on one vital rate and a strong positive effect on a different vital rate (Fig. 3).
As noted by @Freckleton2011, complex age or stage structure can compromise predictions from density-structured models that aggregate over life-histories.
Interactions between a species' response to climate and life-history stages would also impact model forecasts because important climate effects could be averaged-over.
Second, our particular recruitment model is rather coarse and may therefore already average over important climate-influenced processes such as seed germination and establishment.
If we had been able to more precisely model the recruitment process, we might have discovered stronger climate effects -- effects that could be hard to detect from percent cover data.
...
Data Accessibility
------------------
The data used in this paper have been archived on Ecological Archives: http://esapubs.org/archive/ecol/E092/143/. All data and R code necessary to reproduce our work has been deposited on Dryad (link) and is also available on GitHub (http://github.com/atredennick/MicroMesoForecast).
