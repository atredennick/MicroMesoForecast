...
-------------------------
The model for quadrat cover ($\mu$) change from time $t$ to $t+1$ is

\begin{align}
\mu_{jq,t+1} &= \gamma_{j,t} + \phi_{jQ} + \beta_{j,t}x_{jq,t} + \theta_{jk}C_{k,t} \\
Y_{jq,t+1} &\sim \text{LogNormal}(\mu_{jq,t+1}, \tau{j}) \text{T}[0,1]
\end{align}
