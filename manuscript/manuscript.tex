...
\caption{Changes in species' mean cover caused by a 1\% increase in
observed precipitation or temperature as predicted by the
individual-based IPM and the aggregate-based QBM. Note that the change
for POSE due to a precipitation increase predicted by the QBM is almost
zero and so does not show up in the figure.}
\end{figure}
