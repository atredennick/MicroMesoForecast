...
\subsubsection{Vital rate sensitivities to climate
(IPM)}\label{vital-rate-sensitivities-to-climate-ipm}
...
\begin{table}[ht]
\centering
\caption{Percent change from equilibrium (temporal median of simulated plant cover) at observed climate relative to a 1\% climate change as forecast by each model type.} 
\begin{tabular}{llrr}
  \hline
Species & Simulation & IPM Change (\%) & QBM Change (\%) \\ 
  \hline
BOGR & ppt & 0.92 & 19.43 \\ 
  BOGR & temp & 12.50 & -93.56 \\ 
  BOGR & ppt+temp & 25.06 & -27.88 \\ 
  HECO & ppt & -6.65 & -9.40 \\ 
  HECO & temp & -6.46 & -10.63 \\ 
  HECO & ppt+temp & -25.72 & -14.27 \\ 
  PASM & ppt & -8.53 & 0.25 \\ 
  PASM & temp & 8.03 & 8.06 \\ 
  PASM & ppt+temp & 8.74 & 1.30 \\ 
  POSE & ppt & 1.71 & -8.99 \\ 
  POSE & temp & 20.06 & -16.80 \\ 
  POSE & ppt+temp & 11.06 & 4.04 \\ 
   \hline
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics{components/figure/manuscript-unnamed-chunk-1.pdf}
