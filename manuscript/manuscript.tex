...
events were rare in our time series (N=X and N=X, respectively). Given
the broad spatial distribution of the quadrats we are studying, it is
safe to assume that these events are in fact rare enough to be ignored
for our purposes. So we constrained our statistical modeling of vital
rates at the population level to change in percent cover within
...
not including true zeros. However, when we used fitted model parameters
from a beta regression in quadrat-based population model the simulated
population tended toward 100\% cover for all species. We therefore chose
a more-constrained modeling approach based on a truncated log-normal
