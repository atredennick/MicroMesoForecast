...
<<echo=FALSE, include=FALSE, fig=TRUE, label=DevFig, width=5, height=3, results=hide>>=
library(ggplot2)
...
<<echo=FALSE, include=FALSE, fig=TRUE, label=CoefFig, width=8.5, height=10, results=hide>>=
...
\end{center}
\caption{Posterior means (vertical ticks), 75\% credible intervals (heavy lines), and 95\% credible intervals (light lines) of climate effects in each vital rate statistical model. The quadrat level estimates are shown in left panels and individual level estimates are in right panels.}
\label{fig:CoefFig}
\end{figure}
