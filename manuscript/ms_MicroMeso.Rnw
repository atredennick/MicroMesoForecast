...
<<echo=FALSE, include=FALSE, fig=TRUE, label=ClimFig, width=5, height=3>>=
library(ggplot2)
...
ggplot(data=plotD, aes(x=VitalRate, y=value, fill=Level))+
...
\end{center}
\caption{The proportion of interannual variability in  vital rates explained by the climate covariates. The contribution for growth is defined as: (Climate model - Constant Model)/(Full model - Constant model). The contribution for survival and colonization at the quadrat level, where we could not estimate a full model with year random effects, is defined as: (Constant Model - Climate Model)/Constant Model. The values are model deviance estimated from the `coda' package in R.}
\label{fig:ClimFig}
\end{figure}
