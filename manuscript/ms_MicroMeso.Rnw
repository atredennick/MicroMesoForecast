...
\begin{figure}[th!]
\begin{center}
\includegraphics[ width=0.5\textwidth ]{Fig-DevFig}
\end{center}
...
\end{center}
\caption{Posterior means (vertical ticks), 75\% credible intervals (heavy lines), and 95\% credible intervals (light lines) of climate effects in each vital rate statistical model. The dashed vertical line is at 0, indicating no effect.}
\label{fig:CoefFig}
\end{figure}
