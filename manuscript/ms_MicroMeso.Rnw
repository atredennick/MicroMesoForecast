...
<<echo=FALSE, include=FALSE, fig=TRUE, label=DevFig, width=4, height=3>>=
library(ggplot2)
...
gClimCont <- ggplot(data=plotD, aes(x=VitalRate, y=value, fill=Level))+
...
<<echo=FALSE, include=FALSE, fig=TRUE, label=CoefFig, width=8.5, height=8>>=
...
gQ <- ggplot(plotQC, aes(x=species, y=X50., color=rate))+
  ...
  geom_errorbar(aes(ymin=X25., ymax=X75.), width=0, size=1.5, position=position_dodge(width=0.5))+
...
library(gridExtra)
g <- grid.arrange(gQ, gI, ncol=2, nrow=1)
g
@
...
\begin{figure}[th!]
\begin{center}
\includegraphics{Fig-CoefFig}
\end{center}
\caption{Posterior means (points), 75\% credible intervals (heavy lines), and 95\% credible intervals (light lines) of climate effects in each vital rate statistical model. The quadrat level estimates are shown in left panels and individual level estimates are in right panels. Color codes: blue = colonization/recruitment, green = growth, red = survival.}
\label{fig:CoefFig}
\end{figure}
