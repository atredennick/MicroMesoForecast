...
library(xtable)
growthSSRTable <- read.csv("../montana/quadBM/vitalRateRegressions/finalModels/growthClimateContributionTable.csv")
survivalSSRTable <- read.csv("../montana/quadBM/vitalRateRegressions/finalModels/survivalClimateContributionTable.csv")
colonizationSSRTable <- read.csv("../montana/quadBM/vitalRateRegressions/finalModels/colonizationClimateContributionTable.csv")

growthSSRTable[,3:6] <- apply(X = growthSSRTable[,3:6], MARGIN = 2, FUN = function(x){round(x, digits = 2)})
survivalSSRTable[,3:6] <- apply(X = survivalSSRTable[,3:6], MARGIN = 2, FUN = function(x){round(x, digits = 2)})
colonizationSSRTable[,3:6] <- apply(X = colonizationSSRTable[,3:6], MARGIN = 2, FUN = function(x){round(x, digits = 2)})

growthSSRTable$Species <- as.character(growthSSRTable$Species)
survivalSSRTable$Species <- as.character(survivalSSRTable$Species)
colonizationSSRTable$Species <- as.character(colonizationSSRTable$Species)

allTableQBM <- rbind(c("Growth", rep("", times=5)), growthSSRTable, 
                     c("Survival", rep("", times=5)), survivalSSRTable, 
                     c("Colonization", rep("", times=5)), colonizationSSRTable) 
colnames(allTableQBM) <- c("", "N", "Constant model", "Climate model", "Full model", "Contribution of climate covariates")

print(xtable(allTableQBM, align=c("l", "l", "l", "l", "l", "l", "X"), 
