...
get.rpa=function(Rpars,cover,FocalWeather,CompWeather,doYear,focalI){
    ...
    for(i in 1:Nspp){
      if(i==focalI){
        mu[i]=cover2[i]*exp(Rpars$intcpt.yr[doYear,i]+sum(Rpars$betaC[i,]*FocalWeather)+sqrt(cover2[i])%*%Rpars$dd[i]) 
      }else{
        mu[i]=cover2[i]*exp(Rpars$intcpt.yr[doYear,i]+sum(Rpars$betaC[i,]*CompWeather)+sqrt(cover2[i])%*%Rpars$dd[i]) 
      }
    }
