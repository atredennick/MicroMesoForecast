...
for(i in 1:Nspp){
  ...
  Spars$intcpt.yr[,i]=Sdata$X.Intercept..yr
  ...
  Spars$nb[i]=Sdata$crowd[1]
  # get competition X size interactions coefficients
  tmp=which(names(Sdata)=="logarea.crowd")
...
# survival function
survive=function(Spars,doSpp,weather,doYear,sizes,crowding){
   logsizes=log(sizes)
   mu=Spars$intcpt[doSpp]+Spars$intcpt.yr[doYear,doSpp]+(Spars$slope[doSpp]+Spars$slope.yr[doYear,doSpp])*logsizes+
    sum(Spars$clim[doSpp,]*weather)+sum(Spars$slopeXclim[doSpp,]*weather)*logsizes+
    Spars$nb[doSpp]*crowding+Spars$slopeXnb[doSpp]*(logsizes*crowding)
   out=inv.logit(mu)
