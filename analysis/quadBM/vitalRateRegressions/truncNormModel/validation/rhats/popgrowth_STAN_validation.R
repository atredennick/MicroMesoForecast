...
####
####  SUBSET DATA TO EXCLUDE LEAVE-OUT YEAR
####
...
for(do_cov in all_covs){
  clim_covs[,do_cov] <- (clim_covs[,do_cov] - as.numeric(subset(scalers, covariate==do_cov)["means"]))/as.numeric(subset(scalers, covariate==do_cov)["sds"])
}
