...
##  Read in data
#bring in data
allD <- read.csv("../../nmData/SPFL/quadratCover.csv")
allD <- subset(allD, year<41)
# allD <- allD[,2:ncol(allD)] #get rid of X ID column
sppList <- "SPFL"

climD <- read.csv("../../nmData/Climate.csv")
...
growD <- growD_all
clim_covs <- growD[,c("pptLag", "ppt1", "ppt2", "TmeanSum1", "TmeanSum2")]
...
hold_data <- subset(growD, year == 33)
clim_covs_out <- hold_data[,c("pptLag", "ppt1", "ppt2", "TmeanSum1", "TmeanSum2")]
...
####
grow_pasm <- growD_all
clim_vars <- c("pptLag", "ppt1", "ppt2", "TmeanSum1", "TmeanSum2")
n.beta <- 24
