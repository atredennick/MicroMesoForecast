...
cv.fcn <- function(i){
  ...
  df_train$sizepptLag <- df_train$pptLag*log(df_train$area)
  df_train$sizeppt1 <- df_train$ppt1*log(df_train$area)
  df_train$sizeppt2 <- df_train$ppt2*log(df_train$area)
  df_train$sizeTmeanSpr1 <- df_train$TmeanSpr1*log(df_train$area)
  df_train$sizeTmeanSpr2 <- df_train$TmeanSpr2*log(df_train$area)
  ...
  df_hold$sizepptLag <- df_hold$pptLag*log(df_hold$area)
  df_hold$sizeppt1 <- df_hold$ppt1*log(df_hold$area)
  df_hold$sizeppt2 <- df_hold$ppt2*log(df_hold$area)
  df_hold$sizeTmeanSpr1 <- df_hold$TmeanSpr1*log(df_hold$area)
  df_hold$sizeTmeanSpr2 <- df_hold$TmeanSpr2*log(df_hold$area)
  ...
  datalist <- list(N=nrow(df_train), Yrs=Yrs, yid=yid,
                   Covs=ncol(clim_covs), Y=df_train$survives, X=log(df_train$area),
                   C=clim_covs, W=W, G=G, gid=groups, beta_tau=sd.now,
