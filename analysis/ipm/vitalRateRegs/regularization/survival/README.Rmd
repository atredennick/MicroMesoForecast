### README for survival ridge-regression scripts
...
* Species-specific subdirectories contain the longitudinal demographic data for genet survival.

* There are three ``R`` files:
    + ``survival_oos_launcher.R``: reads in demographic survival data and crowding data; compiles Stan model for MCMC fitting; calls ``survival_fxns.R`` to fit training data and calculate the log pointwise predictive density (*lppd*) for the model predicting out-of-sample data. Out-of-sample data is a left-out year from the data.
    + ``survival_fxns.R``: contains functions to call Stan models given a particular left-out year and prior standard deviation for climate covariates.
