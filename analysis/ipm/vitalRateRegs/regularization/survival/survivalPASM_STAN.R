...
clim_covs <- survD[,c("ppt2", "TmeanSpr1", "TmeanSpr2")]
...
datalist <- list(N=nrow(survD), Yrs=Yrs, yid=(survD$year-31),
                 ...
                 C=clim_covs, W=W, G=G, gid=groups, beta_tau=0.1)
pars=c("b2")
...
fit <- stan(file = "survival.stan", data=datalist, init =list(inits[[1]]),
...
#                    sig_G=0.3, b2=rep(0.05,length(clim_covs)))
# 
# # fit <- stan(fit=mcmc_samples, data=datalist, pars=pars,
# #             chains=1,
# #             iter=200, warmup=100, init=list(inits[[1]]))
# 
...
# 
# outfile <- paste("survival_stanfits_PASM.RDS", sep="")
# saveRDS(fit, outfile)
