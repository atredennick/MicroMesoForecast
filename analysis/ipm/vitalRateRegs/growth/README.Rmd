### README for growth regression scripts
...
* There are four ``R`` files:
    + ``crowdingCalculation.R``: calculates the amount crowding experience by a focal genet in a quadrat-year. Results are saved in ``*Crowding.csv`` files to be imported for growth regression fitting script.
    + ``growthAllSpp_STAN.R``: script to read-in and format data, and then fit genet growth regression using Stan (see ``growth.stan`` file for model code).
...
* The ``growth.stan`` file contains a Stan model for fitting the genet growth regression via ‘No-U-Turn’ Hamiltonian Monte Carlo. 
