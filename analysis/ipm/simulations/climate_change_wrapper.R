...
for(ii in 3:length(clim_change)){
  clim_alt <- clim_change[ii]
  filetag <- paste(perc_change[ii], ".csv", sep="")
  ...
  for(ss in 1:n_spp){
    doSpp <- spp_list[ss]
    outfile1<-paste("./results/climchange_varyparams/",doSpp,"_ipm_cover_pptChange", filetag ,sep="")
    outfile2<-paste("./results/climchange_varyparams/",doSpp,"_ipm_density_pptChange", filetag,sep="")
    outfile3<-paste("./results/climchange_varyparams/",doSpp,"_ipm_stableSize_pptChange", filetag,sep="")
    climtag <- "ppt"
    source("ipm_climate_simulations.R", echo = FALSE)
