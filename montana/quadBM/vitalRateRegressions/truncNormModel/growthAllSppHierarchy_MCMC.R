...
for(spp in 1:length(sppList)){
  ...
  sppD$yearID <- sppD$year #for random year offset on intercept
  sppD$group <- substring(sppD$quad, 1, 1)
  sppD$percCover <- sppD$totCover/10000
  sppD$percLagCover <- sppD$lag.cover/10000
  backD <- rbind(backD, sppD)
}#end species loop

growD <- backD[2:nrow(backD),]
naID <- which(is.na(growD$lag.cover)==FALSE)
growD <- growD[naID,]
...
# 
# sppNames <- c(rep(sppList, 13+6+13+4+1+1))
