model{
  ...
  for(i in 1:nObs){
    logit(gMu[i]) <- intYr[spp[i],yrs[i]] + intG[spp[i],grp[i]] + beta[spp[i],yrs[i]]*X[i] + temp1[spp[i]]*TmeanSpr1[i] + temp2[spp[i]]*TmeanSpr2[i] + rain1[spp[i]]*ppt1[i] + rain2[spp[i]]*ppt2[i]  
    p[i] <- gMu[i] * tau[spp[i]]
  ...
  #Climate tracking derived quantities
  for(i in 1:nObs){
    ppt1P[i] <- (logit(gMu[i])-(intYr[spp[i],yrs[i]] + intG[spp[i],grp[i]] + beta[spp[i],yrs[i]]*X[i] + temp1[spp[i]]*TmeanSpr1[i] + temp2[spp[i]]*TmeanSpr2[i] + rain2[spp[i]]*ppt2[i]))*(1/(ppt1[spp[i]]))
  }
#   temp1A1 <- mean(temp1P[sppId1])
#   temp1A2 <- mean(temp1P[sppId2])
#   temp1A3 <- mean(temp1P[sppId3])
#   temp1A4 <- mean(temp1P[sppId4])
