...
pGrow <- readRDS("../vitalRateRegressions/truncNormModel/growthParamsMCMC_withLagPpt.rds")
pGrow2 <- melt(pGrow)
...
pGrowAll <- subset(pGrow, Coef=="gInt"|Coef=="rain1"|Coef=="rain2"|Coef=="rainLag"|Coef=="temp1"|Coef=="temp2"|Coef=="tau"|Coef=="betaSpp"|Coef=="intercept")
...
for(i in 1:length(sppList)){
  ...
  for(yr in 2:yearsN){
    ...
    climate <- subset(climD, year==years[yr-1])[,c(2,3,5,4,6)]
...
resD <- combD2
saveRDS(resD, file = "oneStepResids_NoYearEffects_withLagPpt.rds")
