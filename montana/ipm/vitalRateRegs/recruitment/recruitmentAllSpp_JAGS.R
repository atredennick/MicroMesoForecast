model{
  ...
  for(i in 1:nObs){
    Y[i] ~ dnegbinom(q[i], theta[spp[i]])
    q[i] <- theta[spp[i]] / (theta[spp[i]]+lambda[i])
    lambda[i] <- trueP1[i]*fecundity[i]
    log(fecundity[i]) <- intYr[yrs[i], spp[i]] + intG[grp[i], spp[i]] + dd[spp[i]]*trueP2[i] + temp1[spp[i]]*TmeanSpr1[i] + temp2[spp[i]]*TmeanSpr2[i] + rain1[spp[i]]*ppt1[i] + rain2[spp[i]]*ppt2[i]
  ...
  for(s in 1:nSpp){
    ...
    dd[s] ~ dnorm(0, 1e-6)
    for(y in 1:nYrs){
      intcpt.yr[y,s] ~ dnorm(intcpt.mu[s], intcpt.tau[s])  
    }#end year loop
    for(g in 1:nGrp){
